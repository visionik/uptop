# Product Requirements Document: uptop
...
**Project Description**: uptop is a modern re-interpretation of classic programs like top, htop, and btop.  it provides both a CLI for bot use and a TUI for human use.  It's highly modular and uses plug-ins for almost everything.  The Tui is beautifully built with Texual.   It is designed so that users can write custom "panes" and "collectors" to obtain and display any informationt they want.  Uptop already exists, in this stage of the uptop project we are goign to add new collection and pane/dispay plugins

**I want to build uptop that has the following features:**
1. 1. ping monitoring pane / collector that lets users specify a list of servers to ping-monitor
2. 2. remote hardware discovery.  ssh into machine (linux or windows), find attached hadware, visual alert if anything changes
---
